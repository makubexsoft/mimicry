...
public class P2PClusterNode implements ClusterNode, LocalNode
{
				...
				return Channels.pipeline( new ObjectEncoder(), new ObjectDecoder(), new MessageReceiver(),
						new SimpleChannelHandler()
						{
							...
							public void messageReceived( ChannelHandlerContext ctx, MessageEvent e ) throws Exception
							{
								...
								{
									connectedDataChannels.put( ((NodeInfo) e.getMessage()).getNodeId(),
											ctx.getChannel() );
	...
	private void createDiscoveryChannel()
	{
		...
		discoveryBootstrap = new ConnectionlessBootstrap( new OioDatagramChannelFactory(
				Executors.newCachedThreadPool() ) );
	...
	private void connectTo( final NodeInfo node )
	{
		synchronized ( connectionLock )
		{
			...
			future = controlClientBootstrap
					.connect( new InetSocketAddress( node.getIpAddress(), node.getControlPort() ) );
