...
/**
 * A vector clock implementation to determine causal ordering of events across
 * nodes.
 * 
 * @author Marc-Christian Schulze
 * 
 * @param <T>
 *            The data type that is used to identify the rows of the vector.
 *            Typically you would use the data type with which you uniquely
 *            identify the event sources such as a {@link UUID}.
 */
public class VectorClock<T extends Serializable> implements Serializable, Cloneable
{
	...
	{
		/**
		 * Indicates that the event has been occurred before another event in a
		 * causal ordering.
		 */
		BEFORE,
		/**
		 * Indicates that the event has been occurred after another event in a
		 * causal ordering.
		 */
		AFTER,
		/**
		 * Indicates that the causal order of the two events can't be determined
		 * and therefore they are considered to have been occurred concurrently.
		 */
		CONCURRENTLY
	}
}